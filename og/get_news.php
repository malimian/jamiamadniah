...
if (isset($_GET['category'])) {
    ...
    if (!empty($content)) {
        ...
        $current_sequence = (int)return_single_ans("SELECT MAX(pages_sequence) FROM pages WHERE catid = $cat_id") + 1;
        ...
        foreach ($articles as $article) {
            ...
            if (!empty($article_source) && !empty($article_author) && !empty($article_description) && 
                !empty($article_url) && !empty($article_urlToImage) && filter_var($article_urlToImage, FILTER_VALIDATE_URL)) {
                ...
                if ($isAlreadyPosted == 0) {
                    ...
                    if (!empty($slug) && !((strpos($article_url, 'youtube') !== false))) {
                        ...
                        $insert_result = Insert("INSERT INTO `pages` (
                            ...
                            `pages_sequence`, `isactive`, `featured_image`, `article_url`, `article_author`, `article_read`
                        ) VALUES (
                        ...
                        if ($insert_result) {
                            $current_sequence++; // Increment for next article
                            echo "Article added with sequence $current_sequence<br>";
                        }
                    }
                }
            } else {
