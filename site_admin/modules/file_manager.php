...
if (is_array($objects)) {
        ...
        $del_val = SITE_ADMIN;
        if (($key = array_search($del_val, $objects)) !== false){
            unset($objects[$key]);
        }

    foreach ($objects as $file) {
        if ($file == '.' || $file == '..' || $file == SITE_ADMIN) {
            continue;
        }
        $new_path = $path . '/' . $file;
        if (is_file($new_path)) {
        ...
        } elseif (is_dir($new_path) && $file != '.' && $file != '..' && $file != SITE_ADMIN) {
            $folders[] = $file;
        }
    }
}
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p>
            <b><a href="<?php echo fm_enc($file_url) ?>" download><i class="icon-download"></i> Download</a></b> &nbsp;
...
<table class="table">
    <tr>
<th><label><input type="checkbox" title="Invert selection" onclick="checkbox_toggle()"></label></th>
...
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="icon-chain"></i></a>
<a title="Download" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" download><i class="icon-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
function fm_clean_path($path)
{
    ...
    if(strpos($path, SITE_ADMIN) !== false) exit;
...
function fm_show_header()
{
 ...
 ?>
 <link href="css/modules/file_manager.css" rel="stylesheet">
...
width: 16px;
height: 16px;
background-image : url("modules/images/cbimage.jpg");
}
    </style>
<div id="wrappers">
<?php
